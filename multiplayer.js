(() => {
  ...
  const status = t => { if ($("multiStatus")) $("multiStatus").textContent=t; };
  ...
  const box=document.createElement("section"); box.id="multiBox";
  ...
  function renderRoom(r){
    room=r; $("mpRoom").hidden=false; $("mpRoomCode").textContent=r.code;
    $("multiPlayers").innerHTML=`<strong>${r.players.length}/${r.maxPlayers} joueurs</strong>`+
      r.players.map(p=>`<div class="mp-player">${p.bot?"🤖":"👤"} ${p.name}${p.id===r.hostId?" 👑":""}</div>`).join("");
    $("mpStart").disabled=myId!==r.hostId || r.players.length<2 || r.started;
  }
  ...
  function connect(){
    if(!SERVER_URL){ status("Serveur non configuré — le mode solo fonctionne normalement."); return false; }
    if(ws && ws.readyState===WebSocket.OPEN) return true;
    try { ws=new WebSocket(SERVER_URL); } catch(e){ status("Adresse serveur invalide."); return false; }
    ws.onopen=()=>{ status("Connecté au serveur"); const code=localStorage.getItem("atoumoulin_room_code"); if(code && myId && sessionToken) send({type:"room:reconnect",code,playerId:myId,token:sessionToken}); };
    ws.onclose=()=>{started=false;status("Connexion perdue — le mode solo reste disponible");};
    ws.onerror=()=>status("Impossible de joindre le serveur");
    ws.onmessage=e=>{
      let m; try{m=JSON.parse(e.data)}catch{return}
      if(m.type==="error") return status("⚠️ "+m.message);
      if(m.playerId){myId=m.playerId;localStorage.setItem("atoumoulin_player_id",myId);}
      if(m.token){sessionToken=m.token;localStorage.setItem("atoumoulin_player_token",sessionToken);}
      if(m.room){localStorage.setItem("atoumoulin_room_code",m.room.code);renderRoom(m.room);}
      if(m.type==="game:start"){ started=true; status("Partie lancée"); }
      if(m.type==="game:state"){ started=true; applyGameState(m.state,m.seq||0); }
      if(m.type==="player:bot") status(`${m.name} est maintenant contrôlé par un bot`);
      if(m.type==="chat:message"){
        const x=m.message,d=document.createElement("div");
        d.textContent=`${x.playerName} : ${x.text}`;
        $("multiChat").appendChild(d);$("multiChat").scrollTop=$("multiChat").scrollHeight;
      }
    };
    return true;
  }

  $("mpCreate").onclick=()=>{localStorage.setItem("atoumoulin_name",$("mpName").value);if(connect())send({type:"room:create",name:$("mpName").value,maxPlayers:Number($("mpMax").value)})};
  $("mpJoin").onclick=()=>{localStorage.setItem("atoumoulin_name",$("mpName").value);if(connect())send({type:"room:join",name:$("mpName").value,code:$("mpCode").value})};
  $("mpStart").onclick=()=>send({type:"room:start"});
  $("mpSend").onclick=()=>{const i=$("mpText");if(i.value.trim())send({type:"chat:send",text:i.value});i.value=""};
  $("mpText").addEventListener("keydown",e=>{if(e.key==="Enter")$("mpSend").click()});
  ...
  function hook(name){
    ...
    window[name]=function(...args){
      ...
      return send({type:"game:action",fn:name,args});
    };
  }
  for(const fn of ACTIONS) hook(fn);

  function hookSelection(name){
    const original=window[name];
    if(typeof original!=="function") return;
    window[name]=function(index){
      if(!started) return original.call(this,index);
      return send({type:"game:select",selection:Number(index)});
    };
  }
  ...
  window.AtoumoulinMultiplayer={
    enabled:()=>started,
    sendAction:(fn,args=[])=>send({type:"game:action",fn,args}),
    select:index=>send({type:"game:select",selection:Number(index)}),
    room:()=>room,
    playerId:()=>myId
  };
})();
